...
Staged for the ``[mcmc]`` optional extra (SCOPE.md section 3, slice 2b).
This module imports numpyro at module level, so the jaxbo core never imports
it eagerly: ``jaxbo.__init__`` resolves it lazily on first attribute access.
"""
...
class MultifidelityGPclassifier(MCMCmodel):
    ...
    def model(self, batch):
        ...
        length_L = sample(
            "kernel_length_L", dist.LogNormal(0.0, 1.0), sample_shape=(D,)
        )
        ...
        length_H = sample(
            "kernel_length_H", dist.LogNormal(0.0, 1.0), sample_shape=(D,)
        )
...
class BayesianMLP(MCMCmodel):
    ...
    def model(self, batch):
        ...
        b = sample(
            "b%d_mu" % (num_layers - 1), dist.Normal(np.zeros(D_H), np.ones(D_H))
        )
        ...
        b = sample(
            "b%d_std" % (num_layers - 1), dist.Normal(np.zeros(D_H), np.ones(D_H))
        )
