...
:func:`jaxbo.weights.fit_gmm_constrained`.
"""
...
class MultipleIndependentMFGP(GPmodel):
    """Independent two-fidelity GPs, one per output.

    List-valued kwargs ('params', 'batch', 'norm_const') carry one entry per
    output; entry 0 is the objective and the rest are constraints. Each
    entry follows the :class:`jaxbo.multifidelity.MultifidelityGP` parameter
    and batch layout.
    """
    ...
    def compute_cholesky(
        ...
        """Cholesky factor of one output's joint two-fidelity covariance.

        Args:
            params: Packed hyperparameters for one output.
    ...
    def train(
        ...
        Returns:
            One best packed hyperparameter vector per output.
        """
        best_params = []
