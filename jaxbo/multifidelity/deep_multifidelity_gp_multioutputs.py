...
class DeepMultifidelityGP_MultiOutputs(GPmodel):
    """Independent deep multifidelity GPs, one per output.

    ``options['net_arch']`` selects the feature map ('MLP', 'ResNet', or
    'MomentumResNet'). List-valued kwargs ('params', 'batch', 'norm_const')
    carry one entry per output; entry 0 is the objective and the rest are
    constraints.
    """
    ...
    def __init__(self, options: Dict[str, Any], layers: List[int]):
        ...
        Args:
            options: GPmodel options dictionary; must carry 'net_arch' (plus
                its settings) and 'constrained_criterion'.
    ...
    def compute_cholesky(
        ...
        """Cholesky factor of one output's joint covariance over warped inputs.
    ...
    def predict_all(
        ...
        for k, (params, batch, norm_const) in enumerate(zipped_args):
            ...
            theta_H = np.exp(gp_params[D + 1 : -3])
            # Compute kernels
            k_pp = rho**2 * self.kernel(X_star_nn, X_star_nn, theta_L) + self.kernel(
                X_star_nn, X_star_nn, theta_H
            )
            k_pp = k_pp + np.eye(X_star_nn.shape[0]) * (sigma_n_H + jitter(k_pp))
