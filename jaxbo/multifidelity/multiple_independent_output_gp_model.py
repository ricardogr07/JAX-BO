...
class MultipleIndependentOutputsGP(GPmodel):
    """Independent single-fidelity GPs, one per output.
    ...
    def compute_cholesky(
        ...
        """Cholesky factor of one output's kernel matrix.

        Args:
            params: Log-transformed hyperparameters for one output.
    ...
    def predict_all(
        ...
        for k, (params, batch, norm_const) in enumerate(
            zip(params_list, batch_list, norm_const_list)
        ):
            X, y = batch["X"], batch["y"]
            sigma_n = np.exp(params[-1])
            theta = np.exp(params[:-1])
    ...
    def draw_posterior_sample(self, X_star: np.ndarray, **kwargs: Any) -> np.ndarray:
        """Draw one posterior sample per output at RAW domain points.

        Args:
            X_star: Query points in the RAW domain, shape (N, D).
            **kwargs: List-valued 'params', 'batch', 'norm_const', 'rng_key'
                plus 'bounds'.

        Returns:
            Stacked samples, shape (num_outputs, N).
        """
        params_list = kwargs["params"]
        batch_list = kwargs["batch"]
        bounds = kwargs["bounds"]
        ...
        for params, batch, norm_const, rng_key in zip(
            params_list, batch_list, norm_const_list, rng_key_list
        ):
